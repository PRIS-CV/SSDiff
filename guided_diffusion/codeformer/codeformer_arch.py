...
from guided_diffusion.codeformer.vqgan_arch import *
...
class CodeFormer(VQAutoEncoder):
    ...
    def forward(self, x, y_t=None,
                t=None, w=0.5, detach_16=True, code_only=False, adain=False):
        # t_emb = self.time_embed(timestep_embedding(t, 100)) # (B, 100) --> (B, 3)
        # while len(t_emb.shape) < len(x.shape):
        #     t_emb = t_emb[..., None]
        # t_emb = t_emb.expand(x.shape)

        # x = x + y_t    # concat degraded lq
        # x = x + t_emb  # concat time embeddings
